...
(() => {
  ...
  function encPath(path){ return encodeURIComponent(path).replace(/%2F/g,"/"); }

  async function getFile({owner, repo, path, branch="main", token}){
    must(owner,"Owner hiányzik"); must(repo,"Repo hiányzik"); must(path !== undefined, "Path hiányzik");
    const p = path ? `/${encPath(path)}` : "";
    const url = `${apiBase}/repos/${owner}/${repo}/contents${p}?ref=${encodeURIComponent(branch)}`;
  ...
  async function putTextFile({owner, repo, path, branch="main", token, message, text, sha=null}){
    must(owner,"Owner hiányzik"); must(repo,"Repo hiányzik"); must(path,"Path hiányzik");
  ...
  async function putBinaryFile({owner, repo, path, branch="main", token, message, bytes, sha=null}){
    must(owner,"Owner hiányzik"); must(repo,"Repo hiányzik"); must(path,"Path hiányzik");
    const url = `${apiBase}/repos/${owner}/${repo}/contents/${encPath(path)}`;
    let binary = "";
  ...
  async function getBranch({owner, repo, branch="main", token}){
    const url = `${apiBase}/repos/${owner}/${repo}/branches/${encodeURIComponent(branch)}`;
    return ghFetch(url, token);
  }

  async function testRepo({owner, repo, branch="main", token}){
    must(owner,"Owner hiányzik"); must(repo,"Repo hiányzik");
