...
(() => {
  ...
  function saveDraft() {
    localStorage.setItem(LS.draft, JSON.stringify(tracks));
  }
  ...
  btnAdd.addEventListener("click", async () => {
    try {
      ...
      if (!f && !urlIn)
        return setStatus(addStatus, "Adj meg audio útvonalat/linket, vagy válassz fájlt.", "warn");
      ...
      if (!t.title)
        t.title = (f ? f.name.replace(/\.[^.]+$/,"") : (t.file.split("/").pop() || "")).replace(/\.[^.]+$/,"");
  ...
  async function uploadPendingAudio() {
    ...
    for (const t of tracks) {
      ...
      if (t._pendingFile.size > 900000) {
        throw new Error(
          `A(z) ${t._pendingFile.name} túl nagy a böngészős GitHub feltöltéshez. Tedd fel git push-sal a repo audio/ mappájába, és az adminban csak az útvonalat add meg.`
        );
      }
